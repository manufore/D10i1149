...
library(plyr) # agregación y resumen de datos
library(xlsx) # formato excel
library(agricolae) # paquete estadístico aplicado a RRNN
library(car) # prueba LeveneTest

# Fijar el directorio de trabajo
setwd("C:/Users/manufore/Documents/Analisis Estadistico Macropropagación")
...
#---------------------------------------------------------------------------------------#
# Ensayos de Formación de plantas madres seguimiento (Referencia Temporada T2)
#---------------------------------------------------------------------------------------#

# Agrupación de dos tratamientos de las mismas características
datos$id[datos$id=="E2T1"] <- "E1T2"

# Definición de ensayo Tipo de contenedor (Cruzas 2013)
datos$id2 <- ifelse(datos$setos=="2013" & datos$origen=="CC" & datos$contenedor=="Canaletón" & datos$medicion=="abril2013","E5T2",
...
# definición de filtro de selección Productividad y Enraizamiento
sele <- datos$select1
# Ensayo N1 : Altura de corte #
Alturacorte <- subset(datos, (id2=="E1T1" | id2=="E1T2" | id2=="E1T3") & sele=="SELECCIÓN" & !is.na(produce2) & produce1>=5 & produce2>=5 & enraiza1>=50 & enraiza2>=50)
# Ensayo N2 : Fertilización al sustrato #
Fertilizacion <- subset(datos, (id2=="E1T2" | id2=="E2T2") & sele=="SELECCIÓN" & !is.na(produce2) & produce1>=5 & produce2>=5 & enraiza1>=50 & enraiza2>=50)
# Ensayo N3 : Tipo de contenedor #
Tipocontenedor <- subset(datos, (id2=="E5T1" | id2=="E5T2") & select2=="SELECCIÓN" & !is.na(produce3) & produce2>=5 & produce3>=5 & enraiza2>=50 & enraiza3>=50)
# Ensayo N4 : Tipo de sustrato #
Tiposustrato <- subset(datos, (id2=="E1T2" | id2=="E4T1") & sele=="SELECCIÓN" & !is.na(produce2) & produce1>=5 & produce2>=5 & enraiza1>=50 & enraiza2>=50)

# definición de filtro de selección para análisis de sobrevivencia
# Ensayo N1 : Altura de corte #
...
# Variables a utilizar (producción de miniestacas)
...
# Título de los dataframes de cada ensayo
attr(FormaplantaP[[1]], "titulo") <- "Altura de corte"
attr(FormaplantaP[[2]], "titulo") <- "Fertilización al sustrato"
attr(FormaplantaP[[3]], "titulo") <- "Tipo de contenedor"
attr(FormaplantaP[[4]], "titulo") <- "Tipo de sustrato"

# Reetiquetado para Temporadas de producción-enraiza-svive
for (i in 1:4) {
  FormaplantaP[[i]]$variable <- factor(FormaplantaP[[i]]$variable,
                                       ...
                                       labels = if  (attr(FormaplantaP[[i]], "titulo")=="Tipo de contenedor") { c("Año 1","Año 2") } else { c("Año 1","Año 2","Año 3","Año 4")})[, drop=TRUE]
...
# Definición de etiquetas para tratamientos
...
# Generación de graficos de barra por cada tratamiento
for (i in 1:4) {
  ...
  # Edición final para tablas en Excel
  ...
  TablaFinalP <- cbind(TablaProductividad[TablaProductividad$variable=="Año 1",],TablaProductividad[TablaProductividad$variable=="Año 2",][-c(1,3,4,5)])
  TablaFinalE <- cbind(TablaEnraizamiento[TablaEnraizamiento$variable=="Año 1",],TablaEnraizamiento[TablaEnraizamiento$variable=="Año 2",][-c(1,3,4,5)])
...
#--------------------------------------------------------------------------#
# Comparación de medias Entre tratamientos en temporada respectiva
#--------------------------------------------------------------------------#
for (i in 1:4) {
...
#--------------------------------------------------------------------------------#
# Comparación Productividad de medias Entre temporadas en tratamiento respectivo #
#--------------------------------------------------------------------------------#
...
#---------------------------------------------------------------------------------#
# Comparación Enraizamiento de medias Entre tratamientos en temporada respectiva  #
#---------------------------------------------------------------------------------#
...
#--------------------------------------------------------------------------------#
# Comparación Enraizamiento de medias Entre temporadas en tratamiento respectivo #
#--------------------------------------------------------------------------------#
